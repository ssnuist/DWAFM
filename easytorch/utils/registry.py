...
def scan_modules(current_dir, current_file, exclude_files=None):
    if exclude_files is None:
        exclude_files = []

    module_dir = os.path.dirname(current_file)
    import_prefix = os.path.relpath(module_dir, current_dir).replace(os.sep, ".")
    import_prefix = import_prefix.lstrip(".")

    model_file_names = [
        os.path.splitext(file)[0] for file in os.listdir(module_dir)
    ...
    for file_name in model_file_names:
        try:
            modules.append(importlib.import_module(f'{import_prefix}.{file_name}'))
        except ModuleNotFoundError as e:
            print(f"Failed to import module {file_name}: {e}")
    return modules
