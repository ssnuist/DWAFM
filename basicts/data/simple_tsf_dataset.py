...
class TimeSeriesForecastingDataset(BaseDataset):
    ...
    def _load_data(self) -> np.ndarray:
        ...
        if self.mode == 'train':
            ...
            train_data = data[:train_len + offset].copy()
            return train_data
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        """
        # data = self.data
        history_data = self.data[index:index + self.input_len]  # [input_len, N, F]
        future_data = self.data[index + self.input_len:index + self.input_len + self.output_len] # [Output_len, N, F]
