...
class ZScoreScaler(BaseScaler):
    ...
    def __init__(self, dataset_name: str, train_ratio: float, norm_each_channel: bool, rescale: bool):
        ...
        description_file_path = f'D:/PYTHON_PROJECT/BasicTS-master/datasets/{dataset_name}/desc.json'
        with open(description_file_path, 'r') as f:
            description = json.load(f)
        data_file_path = f'D:/PYTHON_PROJECT/BasicTS-master/datasets/{dataset_name}/data.dat'
    ...
    def get_train_data(self) -> torch.tensor:
        """
        Returns the raw training data used for fitting the scaler.

        Returns:
            np.ndarray: Training data with shape [T_train, N]
        """
        train_data = self.transform(self.train)
        # print(train_data)
